...
export function waitlistSourceKey(fields = {}) {
  const content = norm(fields.utm_content);
  if (BY_CONTENT[content]) return BY_CONTENT[content];

  const medium = norm(fields.utm_medium);
  if (BY_MEDIUM[medium]) return BY_MEDIUM[medium];

  const source = norm(fields.utm_source);
  if (BY_SOURCE[source]) return BY_SOURCE[source];
