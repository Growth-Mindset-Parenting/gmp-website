...
function details(link) {
  const { calendarDescription, meetingId, passcode, oneTapMobile, dialInNumbers } = WORKSHOP.event;
  const parts = [];
  if (link) parts.push(`Your link to join: ${link}`);
  parts.push(calendarDescription);
  parts.push(
    [
      'Rather join by phone?',
      `One tap mobile: ${oneTapMobile.join(' or ')}`,
      '',
