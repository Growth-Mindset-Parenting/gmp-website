...
//   - Except our own buttons (utm_source=website, e.g. the essay pages'
//     waitlist buttons): they only count when we don't already know where
//     the visitor came from. A ManyChat or email tag is never replaced by one.
...
const IGNORED_REFERRERS =
  /(^|\.)(growthmindsetparenting\.com|kajabi\.com|mykajabi\.com|stripe\.com|paypal\.com|zoom\.us|kit\.com|convertkit\.com)$|^localhost$/i;

const clean = (v) => String(v).trim().toLowerCase().slice(0, 100);

// Tags on links inside our own site. They never replace an outside source.
export const INTERNAL_SOURCE = 'website';
const isOutside = (tags) => Boolean(tags?.utm_source) && tags.utm_source !== INTERNAL_SOURCE;
...
export function nextAttribution({ searchParams, referrer, saved, landing }) {
  try {
    ...
    if (tags.utm_source) {
      if (tags.utm_source === INTERNAL_SOURCE && isOutside(saved)) return null;
      return { ...tags, landing: String(landing).slice(0, 100) };
    }
...
// Tags on the current URL win over the cookie, unless the URL's tags are our
// own (internal) and the cookie already holds an outside source.
export function getAttribution() {
  const out = {};
  try {
    const fromUrl = tagsFromSearch(new URLSearchParams(location.search));
    const saved = readCookie();
    const useUrl = fromUrl.utm_source && !(fromUrl.utm_source === INTERNAL_SOURCE && isOutside(saved));
    const source = useUrl ? fromUrl : saved || {};
